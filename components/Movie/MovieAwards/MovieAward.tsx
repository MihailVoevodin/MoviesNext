        ...
        <>
            {awardsArrayFiltered.length > 0 &&
                <div className={styles.awardContainer}>
                    <div className={styles.awardTitle}>{awardType}, {awardsArrayFiltered[0].year} год</div>
                            ...
                            {AwardsArrayWin.map((award: any, id: number) => {
                                return (
                                    <div key={id} className={styles.awardList}>
                                        <CaretRightFilled />
                                        <div>{award.nominationName}
                                            {award.persons.length > 0 &&
                                                <span> ({award.persons.map((person: any, id: number) => {
                                                    return <Link key={id} href={`/name/${person.kinopoiskId}`}>{person.nameRu || person.nameEn}{id !== award.persons.length - 1 ? ', ' : ''}</Link>
                                                })})
                                                </span>
                                            }
                                        </div>
